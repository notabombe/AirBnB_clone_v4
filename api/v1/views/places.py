...
def places_search():
    ...
    if states and len(states) > 0:
        ...
        state_cities = {
            city.id for city in all_cities.values() if city.state_id in states
        }
    else:
        state_cities = set()
    cities = req_json.get('cities')
    if cities and len(cities) > 0:
        cities = {c_id for c_id in cities if storage.get('City', c_id)}
    ...
    if amenities and len(amenities) > 0:
        amenities = {a_id for a_id in amenities if storage.get('Amenity', a_id)}
        for p in all_places:
            p_amenities = None
